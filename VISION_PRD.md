# VISION SECTION — PRD + Full Component
## File: `src/pages/Home/sections/Vision.jsx`
## Background: var(--color-secondary) — dark navy
## Breakpoints: Mobile < 768px | Tablet 768px–1023px | Desktop 1024px+

---

## LAYOUT STRUCTURE PER BREAKPOINT

### Desktop (1024px+)
- Block 1: grid-cols-2 — headline left, description right
- Block 2: grid-cols-3 — three steps inline with chevron separators
- Block 3: flex row — eyebrow label left, belief phrases right

### Tablet (768px–1023px)
- Block 1: grid-cols-2 — still fits, slightly tighter
- Block 2: grid-cols-1 — steps stack vertically, chevrons hidden
- Block 3: flex-col — label top, phrases below in a row

### Mobile (< 768px)
- Block 1: grid-cols-1 — headline full width, description below
- Block 2: grid-cols-1 — fully stacked
- Block 3: flex-col — label + phrases stacked, phrases wrap

---

## COMPLETE COMPONENT

```jsx
function Vision() {
  const steps = [
    {
      number: '01',
      label: 'LISTEN',
      statement: ['We ', 'understand the data', ' before we touch the model.'],
      accentIndex: 1,
      body: 'We map where it lives, how clean it is, what it can and can\'t tell you. Most AI fails here, quietly. We make it the loudest part of the conversation.',
    },
    {
      number: '02',
      label: 'BUILD',
      statement: ['We ', 'build AI that fits', ' your stack and your workflow.'],
      accentIndex: 1,
      body: 'Off-the-shelf demos collapse the moment they meet enterprise complexity. We engineer for your infrastructure, governance, and how your teams actually operate.',
    },
    {
      number: '03',
      label: 'PROVE',
      statement: ['We ', 'prove it in the P&L', ' — not the lab.'],
      accentIndex: 1,
      body: 'Every deployment ships with metrics that matter to your business — revenue lift, hours saved, decisions accelerated. AI that earns its line item.',
    },
  ]

  return (
    <section
      className="py-16 px-5 md:px-10 lg:px-16"
      style={{ background: 'var(--color-secondary)' }}
    >
      <div className="max-w-7xl mx-auto">

        {/* ── BLOCK 1: Vision header — 2-col on tablet+, stacked on mobile ── */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 mb-10 md:mb-12">

          {/* Left — eyebrow + headline */}
          <div>
            <p className="text-label mb-4 tracking-widest" style={{ color: 'var(--color-highlight)' }}>
              OUR VISION
            </p>
            <h2
              style={{
                fontFamily: 'var(--font-heading)',
                fontSize: 'clamp(24px, 3.5vw, 48px)',
                fontWeight: 700,
                lineHeight: 1.15,
                color: '#ffffff',
                marginBottom: '4px',
              }}
            >
              Great AI doesn't start with a model.
            </h2>
            <h2
              style={{
                fontFamily: 'var(--font-accent)',
                fontSize: 'clamp(22px, 3.2vw, 44px)',
                fontWeight: 600,
                fontStyle: 'italic',
                lineHeight: 1.15,
                color: 'var(--color-highlight)',
              }}
            >
              It starts with your data.
            </h2>
          </div>

          {/* Right — description */}
          <div className="flex items-center">
            <p
              className="text-body"
              style={{ color: 'rgba(255,255,255,0.65)', lineHeight: 1.8 }}
            >
              We see AI as a discipline, not a demo. Three things separate the
              deployments that scale from the ones that stall — and we build for
              all three, in this order.
            </p>
          </div>

        </div>

        {/* Thin divider */}
        <div className="mb-8 md:mb-10" style={{ height: '1px', background: 'rgba(255,255,255,0.08)' }} />

        {/* ── BLOCK 2: Three steps ── */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0 mb-8 md:mb-10">
          {steps.map((step, i) => (
            <div
              key={step.number}
              className="flex flex-col gap-4 py-6 md:py-0"
              style={{
                paddingRight: i < 2 ? 'clamp(0px, 3vw, 40px)' : '0',
                paddingLeft: i > 0 ? 'clamp(0px, 3vw, 40px)' : '0',
                borderRight: i < 2 ? '1px solid rgba(255,255,255,0.08)' : 'none',
                borderBottom: i < 2 ? '1px solid rgba(255,255,255,0.08)' : 'none', /* mobile only feel */
              }}
            >
              {/* Step label row */}
              <div className="flex items-center gap-2">
                <span
                  className="text-label tracking-widest"
                  style={{ color: 'var(--color-highlight)' }}
                >
                  {step.number} — {step.label}
                </span>
                {/* Chevron — visible on desktop only */}
                {i < 2 && (
                  <span
                    className="hidden lg:block ml-auto"
                    style={{
                      color: 'rgba(255,255,255,0.2)',
                      fontSize: '18px',
                      lineHeight: 1,
                    }}
                  >
                    ›
                  </span>
                )}
              </div>

              {/* Statement with inline italic accent */}
              <p
                style={{
                  fontFamily: 'var(--font-heading)',
                  fontSize: 'clamp(16px, 1.8vw, 22px)',
                  fontWeight: 600,
                  lineHeight: 1.35,
                  color: '#ffffff',
                  margin: 0,
                }}
              >
                {step.statement[0]}
                <em
                  style={{
                    fontStyle: 'italic',
                    fontFamily: 'var(--font-accent)',
                    color: 'var(--color-highlight)',
                    fontWeight: 400,
                  }}
                >
                  {step.statement[1]}
                </em>
                {step.statement[2]}
              </p>

              {/* Body */}
              <p
                className="text-body-sm"
                style={{ color: 'rgba(255,255,255,0.55)', lineHeight: 1.75 }}
              >
                {step.body}
              </p>
            </div>
          ))}
        </div>

        {/* Thin divider */}
        <div className="mb-8" style={{ height: '1px', background: 'rgba(255,255,255,0.08)' }} />

        {/* ── BLOCK 3: Belief bar ── */}
        <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-8">

          {/* Label */}
          <p
            className="text-label tracking-widest flex-shrink-0"
            style={{ color: 'var(--color-highlight)' }}
          >
            WHAT WE BELIEVE
          </p>

          {/* Phrases */}
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
            <span
              className="text-h4"
              style={{ color: '#ffffff', fontWeight: 400 }}
            >
              Built on your data
            </span>

            <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: '18px' }}>·</span>

            <span
              className="text-h4"
              style={{ color: '#ffffff', fontWeight: 400 }}
            >
              Engineered for your business
            </span>

            <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: '18px' }}>·</span>

            <span
              style={{
                fontFamily: 'var(--font-accent)',
                fontSize: 'clamp(16px, 1.8vw, 22px)',
                fontStyle: 'italic',
                fontWeight: 400,
                color: 'var(--color-highlight)',
              }}
            >
              Measured by your results.
            </span>
          </div>

        </div>

      </div>
    </section>
  )
}

export default Vision
```

---

## Responsive Behaviour Summary

### Block 1 — Vision header
- Mobile: single column — headline stacks above description
- Tablet+: `grid-cols-2` — headline left, description right, `items-center` vertically aligns description
- Both headline lines use `clamp()` for fluid font scaling

### Block 2 — Three steps
- Mobile: `grid-cols-1` — fully stacked, each step separated by subtle bottom border
- Tablet+: `grid-cols-3` — three columns, separated by `1px` right border
- Chevron `›` between steps: `hidden lg:block` — desktop only
- Step padding: `clamp(0px, 3vw, 40px)` — fluid, not cramped on tablet
- Inline italic accent phrase uses `--font-accent` (Playfair Display italic) for visual variety

### Block 3 — Belief bar
- Mobile: `flex-col` — label on top, phrases wrap below
- Tablet+: `flex-row items-center` — label left, phrases inline
- Separator dots: `rgba(255,255,255,0.25)` — subtle, not distracting
- Last phrase italic red accent — matches desktop design exactly

---

